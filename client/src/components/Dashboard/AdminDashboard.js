  ...
  const applyChanges = async (userId) => {
    try {
      ...
      if (changes.delete) {
        ...
        if (changes.role) {
          await api.put(`/api/users/update-role/${userId}`, { role: changes.role });
        }

        if (changes.assignedManager) {
          await api.put(`/api/users/${userId}/assign-manager`, { managerId: changes.assignedManager });
          await api.put(`/api/users/${changes.assignedManager}/assign-client`, { clientId: userId });
        }
        ...
        setMessage(`Changes applied successfully for ${users.find(u => u._id === userId).username}`);
      }
  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className="dashboard-tabs">
        ...
        <button onClick={() => setActiveTab('chatSubmissions')} className={activeTab === 'chatSubmissions' ? 'active' : ''}>Chat Submissions</button>
      </div>
              ...
              {users.map((user) => (
                <tr key={user._id}>
                  ...
                  <td>
                    {user.role === 'client' && (
                      <select
                        value={pendingChanges[user._id]?.assignedManager || user.assignedManager || ''}
                  ...
                  <td>
                    ...
                    {pendingChanges[user._id] && (
                      <button onClick={() => applyChanges(user._id)} className="apply-button">
                        Apply
                      </button>
                    )}
